...
const imageContainer = document.querySelector('.gallery__image-container7');
...
const modalImages = document.querySelector('.modal-gallery__background7');
...
thumnails.forEach(thumnail => {
    thumnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../MEDIA/image-product7-${event.target.id}.png')`
...
const modalImageContainer = document.querySelector ('.modal-gallery__image-container7');
modalThumnails = [...modalThumnails]

modalThumnails.forEach(modalThumnail => {
    modalThumnail.addEventListener('click', event=>{
        console.log(event.target.id.slice(-1));
        modalImageContainer.style.backgroundImage = `url('../MEDIA/image-product7-${event.target.id.slice(-1)}.png')`
    })
});
...
function drawProductInmodal() {
    ...
    <img class="cart-modal__image" src="../MEDIA/image-product7-1.png" alt="">
    <div>
        <p class="cart-modal__product">Cammo TrackSuit</p>
        <p class="cart-modal__price">$800 x3 <span>$2400</span></p>
    </div>
...
<div class="cart-modal__pay-off">
    ...
    pricesModal.innerHTML = `$800 x${lastValue} <span>$${lastValue*800}.00</span>`;
}
...
function changeNextImage(imgContainer) {
    ...
    imgContainer.style.backgroundImage = `url('../MEDIA/image-product7-${imgIndex}.png')`
