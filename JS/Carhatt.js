...
const imageContainer = document.querySelector('.gallery__image-container4');
...
const modalImages = document.querySelector('.modal-gallery__background4');
...
thumnails.forEach(thumnail => {
    thumnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../MEDIA/image-product4-${event.target.id}.png')`
...
const modalImageContainer = document.querySelector ('.modal-gallery__image-container4');
modalThumnails = [...modalThumnails]

modalThumnails.forEach(modalThumnail => {
    modalThumnail.addEventListener('click', event=>{
        console.log(event.target.id.slice(-1));
        modalImageContainer.style.backgroundImage = `url('../MEDIA/image-product4-${event.target.id.slice(-1)}.png')`
    })
});
...
function drawProductInmodal() {
    productContainer.innerHTML = ` 
       <div class="cart-modal__details-container">
                    <img class="cart-modal__image" src="../MEDIA/image-product4-1.png" alt="">
                    <div>
                        <p class="cart-modal__product">Carhatt skeleton Jacket</p>
                        <p class="cart-modal__price">$450 x3 <span>$1.350</span></p>
                    </div>
    ...
    pricesModal.innerHTML = `$450 x${lastValue} <span>$${lastValue*450}.00</span>`;
}
...
function changeNextImage(imgContainer) {
    ...
    imgContainer.style.backgroundImage = `url('../MEDIA/image-product4-${imgIndex}.png')`
