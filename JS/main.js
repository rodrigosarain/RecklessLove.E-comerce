...
thumnails.forEach(thumnail => {
    thumnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../MEDIA/image-product-${event.target.id}.png')`
...
modalThumnails.forEach(modalThumnail => {
    modalThumnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('../MEDIA/image-product-${event.target.id.slice(-1)}.png')`
    })
});
...
function drawProductInmodal() {
    ...
    <img class="cart-modal__image" src="../MEDIA/image-product-1.png" alt="">
    <div>
        <p class="cart-modal__product">Morality Shirt</p>
...
<div class="cart-modal__pay-off">
    ...
    pricesModal.innerHTML = `$125 x${lastValue} <span>$${lastValue*125}.00</span>`;
}
...
function changeNextImage(imgContainer) {
    ...
    imgContainer.style.backgroundImage = `url('../MEDIA/image-product-${imgIndex}.png')`
...
// btn chekout
const checkOutBtn = document.querySelector('.cart-modal__chekout');
